...
def plot_map_global(map_global):   
    ...
    plt.contourf(X, Y, map_global[:, :].T, levels=levels, cmap='coolwarm', extend ='both')  #cmap ='coolwarm','bwr'
    ax.set_aspect('equal')
...
def average_map(maps_dxdy, df_frame, map_dxdy_median, params):
    def avg_map_dxdy_fov(maps_dxdy_fov_norm, params):
        ...
        return avg_map_dxdy_fov_norm
    

    maps_dxdy_norm = maps_dxdy / map_dxdy_median # Normalize avg_map_dxdy

    maps_dxdy_fov_norm_1x = [offset_map_fov(maps_dxdy_norm[frame_idx], df_frame.loc[frame_idx, 'xi_fov_1x'], df_frame.loc[frame_idx, 'yi_fov_1x'], params['map_x_shift'], params['map_y_shift']) \
    ...
    maps_dxdy_fov_norm_4x = [offset_map_fov(maps_dxdy_norm[frame_idx], df_frame.loc[frame_idx, 'xi_fov_4x'], df_frame.loc[frame_idx, 'yi_fov_4x'], params['map_x_shift'], params['map_y_shift']) \
                        for frame_idx in range(len(df_frame)) if (df_frame.loc[frame_idx, 'flag_center_dot_outlier'] == 0) and 
                        (df_frame.loc[frame_idx, 'flag_fov_dot_outlier'] == 0) and (df_frame.loc[frame_idx, 'flag_slope_outlier'] == 0)]
    
    maps_dxdy_fov_norm_xx = {1 : maps_dxdy_fov_norm_1x, 4 : maps_dxdy_fov_norm_4x}
    summary = {}

    for x, maps_dxdy_fov_norm in maps_dxdy_fov_norm_xx.items():
        avg_map_dxdy_fov_norm = avg_map_dxdy_fov(maps_dxdy_fov_norm, params)
        plot_map_norm(avg_map_dxdy_fov_norm, levels=np.linspace(0.985, 1.015, 7), fname_str='Average_'+str(x)+'x_Map_d')
        summary_x = calc_local_ps_kpi(avg_map_dxdy_fov_norm, map_type='avg_'+str(x)+'x_map')
        summary.update(summary_x)      
...
def eval_KPIs(dots, frames, params, summary_old, middle_frame_index, maps_dxdy):
    ...
    # Use both spacings 1x and 4x 
    unitspacing_xy_4x = map_dxdy_median[60, 60] / 4
    unitspacing_xy_1x = map_dxdy_median[60, 60] / 1

    unitspacing_xy = {1 : unitspacing_xy_1x, 4 : unitspacing_xy_4x}
    
    frames['xi_fov_4x'] = ((frames['fov_dot_x'] - frames['center_dot_x']) / unitspacing_xy_4x[0]).astype('float64')
    frames['yi_fov_4x'] = ((frames['fov_dot_y'] - frames['center_dot_y']) / unitspacing_xy_4x[1]).astype('float64')

    frames['xi_fov_1x'] = ((frames['fov_dot_x'] - frames['center_dot_x']) / unitspacing_xy_1x[0]).astype('float64')
    frames['yi_fov_1x'] = ((frames['fov_dot_y'] - frames['center_dot_y']) / unitspacing_xy_1x[1]).astype('float64')

    # Normalized Maps will have 'dxdy_spacing' only, either 1x or 4x
    summary_old['algo_version'] = cf.get_version()
    summary_old['xi_fov'] = (summary_old['fov_dot_x'] - summary_old['center_dot_x']) / unitspacing_xy[params['dxdy_spacing']][0]
    summary_old['yi_fov'] = (summary_old['fov_dot_y'] - summary_old['center_dot_y']) / unitspacing_xy[params['dxdy_spacing']][1]
    ...
    no_center_frame.drop(no_center_frame.loc[(no_center_frame['flag_slope_outlier'] == 1)
                     | (no_center_frame['flag_fov_dot_outlier'] == 1) 
                     | (no_center_frame['flag_center_dot_outlier'] == 1)].index, inplace=True)
    no_center_frame['xi_fov'] = (no_center_frame['fov_dot_x'] - no_center_frame['center_dot_x']) / unitspacing_xy[params['dxdy_spacing']][0]
    no_center_frame['yi_fov'] = (no_center_frame['fov_dot_y'] - no_center_frame['center_dot_y']) / unitspacing_xy[params['dxdy_spacing']][1]
    ...
    try:
        ...
        # Adding gaze calculations
        dots['xi_gaze'] = coeff[1] + coeff[0] * dots['frame_num'].astype('int')
        dots['xphi_gaze'] =  ((np.arctan(dots['xi_gaze'] / 45) * 180) / np.pi).astype('int')
    ...
    merged = dots.merge(no_center_frame, on=['frame_num'])
    ...
    ref = merged.loc[merged['frame_num'] == summary_old.loc[0, 'frame_num']].copy()
    xi_fov = summary_old['xi_fov']
    yi_fov = summary_old['yi_fov']

    # # Plot parallax (x and y)
    # median_dot_spacing = merged['median_dot_spacing'].unique()[:-1]
    # xphi_gaze = merged['xphi_gaze'].unique()[:-1]
    # center_dot_x = (merged['center_dot_x'].unique()[:-1])
    # fig, (axx, axy) = plt.subplots(2, sharex=True)
    # fig.suptitle('Parallax')
    # axx.scatter(xphi_gaze, center_dot_x)
    # axx.grid(color='grey', linestyle='dashed', linewidth=0.2)

    # center_dot_y = (merged['center_dot_y'].unique()[:-1])
    # axy.scatter(xphi_gaze, center_dot_y)
    # axy.grid(color='grey', linestyle='dashed', linewidth=0.2)

    # for ax in (axx, axy):
    #     if ax is axx:
    #         ax.set(xlabel='xphi_gaze', ylabel='center_dot_x')
    #     elif ax is axy:
    #         ax.set(xlabel='xphi_gaze', ylabel='center_dot_y')
    ...
    np.savetxt(os.path.join(cf.output_path, 'map_global.csv'), map_global.T, delimiter=',')
    ref.loc[:, 'xi_rel'] = ref.loc[:, 'xi'] - np.round(params['map_x_shift'] + xi_fov.item())
    ref.loc[:, 'yi_rel'] = ref.loc[:, 'yi'] - np.round(params['map_y_shift'] + yi_fov.item())
    ref.to_csv(os.path.join(cf.output_path, 'drr_radial.csv'))
...
def find_middle_frame(df_frame):   
    ...
    try:
        ...
        logger.exception('Cannot find a suitable middle frame. Will use frame 6 for calculations')
        middle_frame_index = 6
    
    return middle_frame_index
    

def find_outliers(df_frame, params):
    if not params['no_red_circle']:
        ...
        height = df_frame.loc[0, 'frame_height']
        width = df_frame.loc[0, 'frame_width']
        ...
        for i in range(len(df_frame.index)):
            ...
            if (df_frame.loc[i, 'dist_fov_center'] < params['delta_dist_fov_center']) or \
                ...
                ((df_frame.loc[i, 'fov_dot_x'] == df_frame.loc[i, 'frame_width'] / 2) and (df_frame.loc[i, 'fov_dot_y'] == df_frame.loc[i, 'frame_height'] / 2)):
        ...
        df_frame['flag_slope_outlier'] = 0
        df_frame['num_frames_processed'] = len(df_frame.index)
        df_frame['num_center_dot_outlier'] = np.nan
        df_frame['num_fov_dot_outlier'] = np.nan
        df_frame['num_slope_outlier'] = np.nan
        df_frame['num_total_outlier'] = np.nan
