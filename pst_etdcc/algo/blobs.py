...
def dot_pattern(image, height, width, params, frame_num, logger):
    ...
    # (x_target, y_target) : Correct Center dot coordinates
    # Set x_target and y_target as the center of the image unless overriden in params
    if {'x_target', 'y_target'} <= set(params.keys()):
    ...
    frame.center_dot = find_center_dot(frame.dots, x_target, y_target, height, width)
...
def find_center_dot(dots, x_target, y_target, height, width):
    ...
    x_center_box_lower = (0.5 - 0.5 * center_roi) * width
    x_center_box_upper = (0.5 + 0.5 * center_roi) * width
    y_center_box_lower = (0.5 - 0.5 * center_roi) * height
    y_center_box_upper = (0.5 + 0.5 * center_roi) * height
    mean_dot_size = np.mean([dot.size for dot in dots])

    center_box_dots = [dot for dot in dots if (dot.x > x_center_box_lower) and (dot.x < x_center_box_upper) and (dot.y > y_center_box_lower) 
                       and (dot.y < y_center_box_upper) and dot.size > 2 * mean_dot_size]
    #sizes_sorted = sorted(center_box_dots, key=lambda x: x.size, reverse=True) # sort size

    dots_pts = np.asarray([[dot.x, dot.y] for dot in center_box_dots])
    target_center_dot = np.asarray([x_target, y_target])

    dist_2 = np.sum((dots_pts - np.asarray(target_center_dot)) ** 2, axis=1)
    center_dot = center_box_dots[np.argmin(dist_2)]
...
def estimate_fov(params, total_frames, height, width, frame_num):
    # Estimate the location of the fov_dot using the experimental display settings
    display_center_x = width / 2
    display_center_y = height / 2
    degree_per_frame = (params['gaze_end'] - params['gaze_start']) / total_frames

    fov_x = display_center_x + params['dot_spacing'] * np.tan((params['gaze_start'] + int(frame_num) * degree_per_frame) * (np.pi / 180)) * 45
    fov_y = display_center_y
    fov_size =  7 # assumed dot size
...
def prep_image(image, params, normalize_and_filter, binarize):
    ...
    if normalize_and_filter:
        ...
        gray = cv2.bilateralFilter(gray, 7, 300, 300)
    if binarize:
