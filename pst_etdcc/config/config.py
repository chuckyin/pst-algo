...
def config(dataset_folder, params_file):
    ...
    params = {'min_dist' : 3,                    # Minimum distance in pixels between pixel group centers from several binary (thresholded) images above which they are considered as distinct blobs
              ...
              'dot_spacing' : 25,                # Default dot spacing in equal projection space
              'gaze_start' : -25,                # Starting gaze angle in angular space (-25 to 25 degrees is the default setting)
              'gaze_end' : 25,                   # Ending gaze angle in angular space (-25 to 25 is the default setting)
              'no_red_circle': False}            # Enables KPI computations for input data without FOV dot detection
