...
def pipeline(queue, df_lst, df_frame_lst, frame_nums, maps_xy, maps_dxdy, output_path, total_frames, params, image_file):
    #------Logging------
    ...
    frame_num = image_file.split(os.path.sep)[-1].split('.tiff')[0].split('0', 1)[-1].lstrip('0')
    ...
    if ('1001.tiff' not in image_file) and ('1000.tiff' not in image_file):
        if params['no_red_circle']:
            true_red_circle = False
            fov_dot = blobs.estimate_fov(params, total_frames, height, width, frame_num)
            logger.info('Frame %s: FOV dot was estimated at %s', frame_num, fov_dot.__str__())
            image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
            true_red_circle = True
    ...
    mini_df_frame = pd.DataFrame({'frame_num': frame_num,
                                  ...
                                  'fov_dot_x' : fov_dot.x, 'fov_dot_y' : fov_dot.y, 'red_circle_present' : true_red_circle,
                                  ...
                                  'map_x_shift' : params['map_x_shift'], 'map_y_shift' : params['map_y_shift'],
                                  'frame_height' : height, 'frame_width' : width}, index=[0])
...
if __name__ == '__main__':
    ...
    for dix, dataset_folder in enumerate(os.listdir(dataset_folders_path)):
        if (not dataset_folder.startswith('.')) and ('output' not in dataset_folder) and \
            ...
            total_frames = len(image_files_all)
            ...
            if params['num_frames'] == 10:
                ...
                image_files = [image_file for image_file in image_files_all 
                            if ((image_file.split(os.path.sep)[-1].split('_'))[-1].split('.tiff'))[0] in frame_num_list]
            elif params['num_frames'] == 17:
                frame_num_list = ['1', '15', '30', '45', '60', '65', '70', '75', '80', '85', '90', '95', '100', '105', '120', '135', '150']
                image_files = [image_file for image_file in image_files_all 
                            #if ((image_file.split(os.path.sep)[-1].split('_'))[-1].split('.tiff'))[0] in frame_num_list]
                             if image_file.split(os.path.sep)[-1].split('.tiff')[0].split('0', 1)[-1].lstrip('0') in frame_num_list]
            else:
            ...
            pipeline_partial = partial(pipeline, queue, df_lst, df_frame_lst, frame_nums, maps_xy_dct, maps_dxdy_dct, cf.output_path, total_frames, params)
            ...
            df.sort_values(by='frame_num', key=lambda x: x.astype('int'), inplace=True, ignore_index=True)
            ...
            if summary_df.empty:
                kpi.find_outliers(df_frame, params)
            ...
            summary_df = kpi.eval_KPIs(df, df_frame, params, summary_df, int(middle_frame_index), maps_dxdy)
            if ~summary_df.empty:
