        ...
        for j in range(yi):
            for k in range(dim):
                if np.count_nonzero(~np.isnan(maps_arr[:,i,j,k]))>=min_num:
                    ...
                    output[i,j,k] = np.nan
    print ('Median map calculation completed with ' , str(cnt), ' dots' )                
    return output
...
def plot_map_norm(map_norm, output_path, fname_str=''):
...
def plot_map_global(map_global, output_path, fname_str=''):
...
def main(argv):
    ...
    for image_file in image_files:
        ...
        if fov_dot is None:
            ...
            bd.draw_dots(image, [fov_dot], filename=frame_num+'_fov.tiff') # For debugging FOV dot detection
        
        bd.draw_dots(image, frame.dots, filename=frame_num+'_dots.tiff') # For debugging blob detection
        ...
        for i in range(len(df_frame)):
            ...
            df_frame.loc[i, 'd_center_dots'] = np.sqrt((df_frame['center_dot_x'].iloc[i] - median_center_x)**2 + (df_frame['center_dot_y'].iloc[i] - median_center_y)**2)
            ...
            df_frame.loc[i, 'd_fov_center'] = np.sqrt((df_frame['fov_dot_x'].iloc[i] - df_frame['center_dot_x'].iloc[i])**2 + (df_frame['fov_dot_y'].iloc[i] - df_frame['center_dot_y'].iloc[i])**2)        
    ...
    try: 
        plot_map_norm(map_dxdy_norm_fov, cf.output_path)
    except ValueError:
    ...
    for i in [0,-1]: #first and last frame
        ...
        try:
            plot_map_global(map_global, cf.output_path, fname_str= label)
        except ValueError:
